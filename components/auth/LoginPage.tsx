...
export default function LoginPage({ params, dict }: { params: { lang: string }, dict: any }) {
    const { lang } = params;
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <div className="h-6 w-48 bg-white/40 rounded-md" />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white/5 rounded-xl p-4 space-y-3">
                        ...
                        <div className="h-6 w-12 bg-white/50 rounded-md" />
                    </div>
                    <div className="bg-white/5 rounded-xl p-4 space-y-3">
                ...
                <div className="h-32 bg-gradient-to-br from-primary/20 to-purple-500/20 rounded-xl w-full" />
            </div>

            {/* Graphic text */}
            <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
                <h3 className="text-2xl font-bold text-white text-center px-4 leading-relaxed">
                    {dict.Auth?.joinNow || "Manage your AI journey."}
                </h3>
            </div>
        </div>
    );
    ...
    return (
        <AuthLayout
            ...
            subtitle={dict.Auth.enterDetails}
            ...
            <form action={formAction} className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <input type="checkbox" id="remember" className="rounded border-white/20 bg-white/5 text-primary focus:ring-primary/50" />
                        <label htmlFor="remember" className="text-xs text-muted-foreground cursor-pointer select-none">{dict.Auth.rememberMe}</label>
                    </div>
                ...
                <button
                    type="submit"
                    disabled={isPending}
                    className="w-full bg-primary hover:bg-primary/90 text-white font-semibold py-3 rounded-full transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/20 hover:shadow-primary/30 active:scale-[0.98]"
                >
                ...
                <p className="text-center text-xs text-muted-foreground mt-8">
                    {dict.Auth.dontHaveAccount} <Link href={`/${lang}/register`} className="text-primary hover:underline">{dict.Auth.signUpLink}</Link>
                </p>
