import { getHooksByCategory } from "@/lib/db";
import { getDictionary } from "@/lib/dictionary";
import { Sidebar } from "@/components/layout/Sidebar";
import { TopNavbar } from "@/components/layout/TopNavbar";
import { getSession } from "@/lib/session";
import { HookCard } from "@/components/ui/HookCard";
import { Anchor, Search, Filter } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; category: string }> }): Promise<Metadata> {
    ...
    const category = await prisma.category.findFirst({
        where: { slug: categorySlug, type: 'hook' }
    });

    if (!category) return { title: 'Category Not Found' };

    return {
        title: category.metaTitle || `${category.name} Hooks | Content Marketing | Promptda`,
        description: category.metaDescription || `Explore our collection of ${category.name} viral hooks. Increase your engagement and CTR with proven strategies.`,
        alternates: {
            canonical: category.canonicalUrl || `https://promptda.com/${lang}/hooks/${categorySlug}`
        },
        robots: {
...
export default async function CategoryHooksPage({
    params
}: {
    ...
    const hooks = await getHooksByCategory(categorySlug);
    ...
    const breadcrumbs = [
        ...
        { name: category.name, item: `https://promptda.com/${lang}/hooks/${categorySlug}` }
    ];
                    ...
                    <section className="relative p-12 overflow-hidden rounded-[40px] bg-gradient-to-br from-emerald-500/10 via-background to-background border border-emerald-500/10">
                        <div className="relative z-10 space-y-4">
                            ...
                            <nav className="text-xs font-bold text-muted-foreground uppercase tracking-widest absolute top-12 right-12 hidden md:block">
                                <Link href={`/${lang}`} className="hover:text-emerald-500 transition-colors">Home</Link>
                                <span className="mx-2">/</span>
                                <Link href={`/${lang}/hooks`} className="hover:text-emerald-500 transition-colors">Hooks</Link>
                                <span className="mx-2">/</span>
                            ...
                            <h1 className="text-5xl font-black text-foreground tracking-tighter leading-none">
                                {category.headline || (
                                    <>
                                        {category.name} <span className="text-emerald-500 italic">Hooks</span>
                                    </>
                                )}
                            </h1>
                            <p className="text-lg text-muted-foreground font-medium max-w-2xl leading-relaxed">
                                {category.description || `High-converting ${category.name} hooks and content strategies to dominate social media algorithms.`}
                            </p>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                <span className="font-black text-foreground">{hooks.length}</span> Hooks Found
                            </div>
                        </div>
                        ...
                        {hooks.length === 0 && (
                            <div className="col-span-full py-32 text-center space-y-4">
                                ...
                                <div className="space-y-1">
                                    <h3 className="text-xl font-bold">No hooks in this category yet</h3>
